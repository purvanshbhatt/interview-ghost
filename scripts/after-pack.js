 ...
 *
 * Opt-in via GHOST_BUNDLE_WHISPER=1 (or legacy CUE_BUNDLE_WHISPER=1). Preparing the runtime downloads a pinned
...
module.exports = async function afterPack(context) {
  if (!process.env.GHOST_BUNDLE_WHISPER && !process.env.CUE_BUNDLE_WHISPER) {
    console.log('[ghost] Skipping the bundled whisper runtime (set GHOST_BUNDLE_WHISPER=1 to include it).');
    return;
  }
