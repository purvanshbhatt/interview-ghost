...
if (process.platform !== 'win32') {
  // Self-heal for a node_modules shared between OSes (dual-boot): a Windows
  // postinstall leaves path.txt pointing at the masqueraded exe, which breaks
  // `electron .` on Linux/macOS even though the ELF/Mach-O binary is present.
  const pathTxt = path.join(__dirname, '..', 'node_modules', 'electron', 'path.txt');
  try {
    if (fs.existsSync(pathTxt) && fs.readFileSync(pathTxt, 'utf8').trim() !== 'electron') {
      fs.writeFileSync(pathTxt, 'electron');
      console.log('[postinstall] path.txt -> electron (non-Windows host)');
    }
  } catch (_) { /* best effort */ }
  process.exit(0);
}
...
const candidates = ['electron.exe', 'RuntimeBroker.exe', 'SearchHost.exe', 'ghost.exe', 'cue.exe'];
let src = null;
