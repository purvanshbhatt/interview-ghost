...
function startAppLink(deps) {
  ...
  link = new AppLinkServer({
    appId: 'com.ghost.overlay', // Matches electron-builder.cjs. Read off a build, not guessed.
    appSlug: 'ghost',
    appName: 'ghost',
  ...
  link.on('error', (error) => {
    console.log('[ghost] app link error:', error && error.message);
  });

  link.start().catch((error) => {
    // A link that will not start must never stop Ghost from starting. The app
    // worked without this yesterday.
    console.log('[ghost] app link unavailable:', error && error.message);
    link = null;
  });
