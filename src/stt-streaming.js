...
// ============================================================================
// Gemini 3.5 Transcribe Live (WebSocket Streaming STT)
// Uses Google's Gemini Live API for real-time speech transcription
// ============================================================================

class GeminiTranscribeLiveSTT {
  constructor(apiKey, options = {}) {
    this.apiKey = apiKey;
    this.model = options.model || 'gemini-3.5-transcribe-live';
    this.ws = null;
    this.connected = false;
    this.reconnecting = false;
    this.onTranscript = options.onTranscript || (() => {});
  ...
  async connect() {
    ...
    try {
      ...
      this.ws.on('open', () => {
        ...
        // Send Gemini Live Transcription setup message
        const setupMsg = {
          setup: {
      ...
      this.ws.on('message', (data) => {
        try {
          ...
          if (content?.interimInputTranscription?.text) {
            this.onInterim(content.interimInputTranscription.text);
          }
      ...
      this.ws.on('error', (err) => {
        this.onError({ provider: 'gemini-transcribe-live', message: err.message, status: null });
      });
    } catch (e) {
      this.onError({ provider: 'gemini-transcribe-live', message: e.message, status: null });
    }
  }
  ...
  _attemptReconnect() {
    if (this._reconnectAttempts >= this._maxReconnectAttempts) {
      this.onError({ provider: 'gemini-transcribe-live', message: 'Max reconnection attempts reached', status: null });
      return;
    }
    this.reconnecting = true;
    this._reconnectAttempts++;
...
// Priority: Deepgram > Gemini Live / OpenAI Realtime > Batch fallback
// ============================================================================
...
function createStreamingSTT(settings, channel, callbacks) {
  ...
  // Priority 2: Gemini 3.5 Transcribe Live (if gemini-transcribe is selected)
  if (selectedProvider === 'gemini-transcribe' && keys.gemini) {
    const stt = new GeminiTranscribeLiveSTT(keys.gemini, {
    ...
    return { type: 'streaming', provider: 'gemini-transcribe', instance: stt };
  }

  // Priority 3: OpenAI Realtime API (excellent quality)
  ...
  // Priority 4: Gemini Live in auto mode if gemini key is present and deepgram/openai are not
  if (selectedProvider === 'auto' && keys.gemini && !keys.deepgram && !keys.openai) {
    const stt = new GeminiTranscribeLiveSTT(keys.gemini, {
      model: 'gemini-3.5-transcribe-live',
      onTranscript: (text) => onTranscript(channel, text),
      onInterim: (text) => onInterim(channel, text),
  ...
  // Priority 5: Batch fallback (Gemini or Whisper via old system)
  return {
    type: 'batch',
